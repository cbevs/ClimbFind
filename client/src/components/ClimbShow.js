...
const ClimbShow = (props) => {
  ...
  if (props.user) {
    if (props.user.id === climb.userId) {
      editButton = <p onClick={changePaneEdit} className="area-climb-button climb-show-buttons">Edit Climb</p>
    }
  }
  ...
  if (showTicklist === 0) {
    ...
    <h2 className="show-h2">{climb.name}</h2>
      ...
      <p onClick={changePane} className="area-climb-button climb-show-buttons">Log ascent</p>
      {editButton}
    </>
    ...
    pane = 
    <>
      <ClimbEdit changePaneEdit={changePaneEdit} climb={climb} setClimb={setClimb} setParentFeatures={setFeatures}/>
    </>
  }
